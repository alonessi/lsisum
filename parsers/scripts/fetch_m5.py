"""CLI: fetch M5 data (CBR bliquidity + СОРС + Roskazna ЕКС index)."""
...
from ru_liquidity_sentinel_parsers.parsers import m5_treasury
...
@click.option("--skip-sors", is_flag=True, default=False)
@click.option("--skip-roskazna", is_flag=True, default=False)
@click.option("--roskazna-pages", default=10, type=int,
              help="How many index pages to crawl from Roskazna (10 entries each).")
@click.option("--out-raw", default=None)
@click.option("--out-processed", default=None)
def main(date_from: str, date_to: str | None,
         skip_sors: bool, skip_roskazna: bool, roskazna_pages: int,
    ...
    files = m5_treasury.fetch_all(
        date_from=df, date_to=dt,
        skip_sors=skip_sors,
        skip_roskazna=skip_roskazna,
        roskazna_pages=roskazna_pages,
        out_raw=raw, out_processed=proc,
    )
    click.echo(f"M5 done. Files: {files}")
