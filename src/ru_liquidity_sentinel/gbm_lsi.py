...
def shap_module_attribution(model: Any, X: pd.DataFrame, feature_to_module: Dict[str, str],
    ...
    try:
        # Используем аналитический NLL вместо SHAP (работает безупречно для автоэнкодеров/форкастинга)
        if hasattr(model, 'get_nll_components'):
            print("Используем точную аналитическую NLL атрибуцию...")
            nll_components = model.get_nll_components(X)
            # Берем модуль NLL, так как нас интересует вклад в общую амплитуду ошибки
