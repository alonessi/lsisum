...
def module_score(features: pd.DataFrame, module: str) -> pd.Series:
    ...
    flag_block = features[cols["flag"]].astype(float).sum(axis=1).fillna(0.0) if cols["flag"] else pd.Series(0.0, index=features.index)
    mio_block = features[cols["mio"]].mean(axis=1).fillna(0.0) if cols["mio"] else pd.Series(0.0, index=features.index)

    score = (weights["mad_weight"] * mad_subindex + weights["flag_weight"] * flag_block + weights["mio_weight"] * mio_block).clip(lower=0.0, upper=100.0)
...
def fit_model_suite(
    ...
    def _eval_and_store(name: str, model_obj: Any, **kwargs: Any) -> None:
        ...
        pred_te_raw = pd.Series(model_obj.predict(X_test), index=test_idx) if not X_test.empty else pd.Series(dtype=float)
...
def detect_stress_episodes(lsi: pd.Series, threshold: float | None = None, min_episode_days: int = 5, merge_gap_days: int = 5, top_k: int | None = None) -> pd.DataFrame:
    ...
    for ts in dates[1:]:
        if (ts - cur_end).days <= merge_gap_days: cur_end = ts
        else:
            episodes.append((cur_start, cur_end))
            cur_start = cur_end = ts
    episodes.append((cur_start, cur_end))

    rows = [{"start": st, "end": en, "length_days": int((en - st).days) + 1, "max_lsi": float(s.loc[st:en].max()), "mean_lsi": float(s.loc[st:en].mean())} for st, en in episodes if int((en - st).days) + 1 >= min_episode_days]
...
def noise_breakdown(lsi_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [{"term": col.replace("contrib_", ""), "component": "var", "contribution": float(contribs[col].var(ddof=0))} for col in contrib_cols]
...
__all__ = ["MODULE_SCORE_FORMULA", "module_columns", "module_score", "all_module_scores", "apply_m4_penalty", "exponential_smoothing", "build_lsi_frame", "lsi_status", "aggregator_feature_columns", "fit_model_suite", "ModelResult", "MultiModelResult", "sensitivity_analysis", "detect_stress_episodes", "noise_breakdown"]